...
\todo{Из примера видно, что по-шугнансик можно написать и *абед*, и *обед*, однако дальше в обработке, второй вариант теряется. Кроме того, непонятно, зачем делить материал по частям речи, а не сделать одну таблицу в три колонки и дальше из нее делать `.lexd`.}
